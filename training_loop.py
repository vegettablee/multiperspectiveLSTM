...
def start_training_loop(): 
  ...
  if load_model is False: # makes a new model from scratch
    ...
    if load_from_checkpoint is True: # resume training 
      model, optimizer, start_epoch, batch_idx = load.load_checkpoint(model, optimizer)
  ...
  batches = fetchData(BATCH_SIZE) # returns as many batches as there are in the data
  ...
  for epoch in range(start_epoch, EPOCH_SIZE):
    for batch_idx, batch in enumerate(batches):
      ...
      total_loss.backward() 
      optimizer.step()
      load.save_checkpoint(model, optimizer, epoch, batch_idx)# save model at each batch/epoch as a checkpoint 
